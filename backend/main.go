...
func Routers() {
	...
	http.ListenAndServe(":9090", &CORSRouterDecorator{R: router})
}
...
func GetUsers(w http.ResponseWriter, r *http.Request) {
	...
	result, err := db.Query("SELECT id, judul_buku, jumlah, nama_peminjam, alamat_peminjam, nohp_peminjam, tanggal_pinjam, tanggal_pengembalian, lama_pinjam FROM peminjamanbuku_zidan")
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
...
func CreateUser(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	stmt, err := db.Prepare("INSERT INTO peminjamanbuku_zidan(judul_buku," +
...
func GetUser(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	params := mux.Vars(r)
	result, err := db.Query("SELECT id, judul_buku, jumlah, nama_peminjam, alamat_peminjam, nohp_peminjam, tanggal_pinjam, tanggal_pengembalian, lama_pinjam FROM peminjamanbuku_zidan WHERE id = ?", params["id"])
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
...
func UpdateUser(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	params := mux.Vars(r)
	stmt, err := db.Prepare("UPDATE peminjamanbuku_zidan SET judul_buku = ?," + 
...
func DeleteUser(w http.ResponseWriter, r *http.Request) {
	...
	stmt, err := db.Prepare("DELETE FROM peminjamanbuku_zidan WHERE id = ?")
	if err != nil {
		panic(err.Error())
	}
...
func InitDB() {
	db, err = sql.Open("mysql", "root:@tcp(127.0.0.1:3306)/db_2201075_zidan_uas_pilkomb")
	if err != nil {
		panic(err.Error())
	}
}
